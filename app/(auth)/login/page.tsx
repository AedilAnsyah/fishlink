  ...
  return (
    <div className="bg-white py-8 px-6 shadow-sm border border-ink-200 rounded-2xl sm:px-10">
      ...
      <div className="mb-6 p-4 bg-sky-50 border border-sky-200 rounded-xl space-y-3">
        <p className="text-xs font-bold text-ocean-900 flex items-center gap-1.5">
          <ShieldCheck className="w-4 h-4 text-sky-400" />
          Akun Test untuk Demo & Evaluasi
        </p>

        <div className="space-y-2">
          {/* Buyer Test Account */}
          <div
            className="p-3 bg-white border border-ink-200 rounded-lg cursor-pointer hover:border-ocean-900 hover:shadow-sm transition-all group"
            onClick={() => fillTestAccount("buyer")}
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <div className="w-9 h-9 rounded-lg bg-ocean-900 text-white flex items-center justify-center shrink-0">
                  <Building2 className="w-4.5 h-4.5" />
                </div>
                <div>
                  <p className="text-xs font-bold text-ink-900">Pembeli (Buyer)</p>
                  <p className="text-[10px] text-ink-700">{TEST_ACCOUNTS.buyer.name} — {TEST_ACCOUNTS.buyer.business}</p>
                </div>
              </div>
              <Button
                type="button"
                variant="outline"
                size="sm"
                onClick={(e) => { e.stopPropagation(); handleQuickDemoLogin("buyer"); }}
                className="text-[10px] h-7 px-2.5 border-ocean-900 text-ocean-900 hover:bg-sky-100 font-bold"
              >
                Masuk Langsung →
              </Button>
            </div>
            <div className="mt-2 pt-2 border-t border-ink-100 flex gap-4 text-[10px] font-mono text-ink-600">
              <span>Email: <strong className="text-ink-900">{TEST_ACCOUNTS.buyer.email}</strong></span>
              <span>Pass: <strong className="text-ink-900">{TEST_ACCOUNTS.buyer.password}</strong></span>
            </div>
          </div>
          ...
          <div
            className="p-3 bg-white border border-ink-200 rounded-lg cursor-pointer hover:border-ocean-900 hover:shadow-sm transition-all group"
            onClick={() => fillTestAccount("supplier")}
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <div className="w-9 h-9 rounded-lg bg-success-600 text-white flex items-center justify-center shrink-0">
                  <Anchor className="w-4.5 h-4.5" />
                </div>
                <div>
                  <p className="text-xs font-bold text-ink-900">Supplier (Nelayan)</p>
                  <p className="text-[10px] text-ink-700">{TEST_ACCOUNTS.supplier.name} — {TEST_ACCOUNTS.supplier.business}</p>
                </div>
              </div>
              <Button
                type="button"
                variant="outline"
                size="sm"
                onClick={(e) => { e.stopPropagation(); handleQuickDemoLogin("supplier"); }}
                className="text-[10px] h-7 px-2.5 border-success-600 text-success-600 hover:bg-success-100 font-bold"
              >
                Masuk Langsung →
              </Button>
            </div>
            <div className="mt-2 pt-2 border-t border-ink-100 flex gap-4 text-[10px] font-mono text-ink-600">
              <span>Email: <strong className="text-ink-900">{TEST_ACCOUNTS.supplier.email}</strong></span>
              <span>Pass: <strong className="text-ink-900">{TEST_ACCOUNTS.supplier.password}</strong></span>
            </div>
          </div>
        </div>

        <p className="text-[10px] text-ink-400 text-center">
          Klik kartu untuk mengisi otomatis, atau klik &quot;Masuk Langsung&quot; untuk akses instan.
        </p>
      </div>
